...
bool is_user_mode()
{
  // test bit 15 of PSR, if it is 1 we are in user mode, if it is 0 we are in supervisor mode
  if ((reg[PSR] >> 15) & 0x1)
  {
    return true;
  }
...
void user_mode()
{
  // set bit 15 to 1 to indicate user mode
  reg[PSR] |= 0x8000;
}
...
{
  // set bit 15 to 0 to indicate supervisor mode
  reg[PSR] &= 0x7FFF;
}
...
uint16_t priority()
{
  // get bits 10-8 of PSR and return as priority level
...
void set_priority(uint16_t p)
{
  ...
  // set new priority bits, only least 3 significant bits of p should be used
  reg[PSR] |= (p & 0x7) << 8;
}
...
void enable_clock()
{
  // set MCR run latch bit [15] to 1 to enable clock run
  reg[MCR] |= 0x8000;
}
...
void disable_clock()
{
  // set MCR run latch bit [15] to 0 to disable clock run
  reg[MCR] &= 0x7FFF;
}
...
bool is_running()
{
  // test MCR run latch bit [15], if it is 1 the clock is enabled and machine
  // is running, if it is 0 the clock is disabled and machine is not running
  if ((reg[MCR] >> 15) & 0x1)
  {
    return true;
  }
